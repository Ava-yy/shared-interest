...
from work.datasets_prep import get_dataset, get_transform
...
result_list = json.load(open(result_save_dir / ('result_imagenet_s_list_'+ f'_{args.percentile}_{args.num_std}.json'), 'r'))
...
def flatten(json_result):
    ...
    for k in keys:
        ...
        res[k] = data
    predictions = np.array([d['top5_prediction'][0] for d in json_result])

    return res,predictions

result_dict,predictions = flatten(result_list)


with open(str(Path(args.output_dir) / 'bins/image_id.json'), 'w') as f:
    json.dump(result_dict['image_id'].tolist(), f, indent=2)

result_dict['label_id'].astype(np.int32).tofile(str(Path(args.output_dir) /'bins/label_id.bin'))
result_dict['concept_id'].astype(np.int32).tofile(str(Path(args.output_dir) /'bins/concept_id.bin'))
predictions.astype(np.int32).tofile(str(Path(args.output_dir) /'bins/prediction_id.bin'))
result_dict['iou'].astype(np.float32).tofile(str(Path(args.output_dir) /'bins/iou.bin'))
result_dict['gtc'].astype(np.float32).tofile(str(Path(args.output_dir) /'bins/gtc.bin'))
result_dict['sc'].astype(np.float32).tofile(str(Path(args.output_dir) /'bins/sc.bin'))
result_dict['num_components_sc'].astype(np.float32).tofile(str(Path(args.output_dir) / 'bins/num_components_sc.bin'))
result_dict['num_components_gt'].astype(np.float32).tofile(str(Path(args.output_dir) / 'bins/num_components_gt.bin'))


num_comp_gt_list = []
num_comp_sc_list = []

area_of_comp_gt_list = []
area_of_comp_sc_list = []
area_of_comp_gt_ratio_list = []
area_of_comp_sc_ratio_list = []

for item in result_list:
    area_comp_gt = np.array(item['area_of_components_gt']).sum()
    area_comp_sc = np.array(item['area_of_components_sc']).sum()
    area_comp_ratio_gt = np.array(item['area_of_components_gt_ratio']).sum()
    area_comp_ratio_sc = np.array(item['area_of_components_sc_ratio']).sum()
    # num_comp_gt = item['num_components_gt']
    # num_comp_sc = item['num_components_sc']
    area_comp_sc_ratio_list.append(area_comp_ratio_sc)
    area_comp_gt_ratio_list.append(area_comp_ratio_gt)
    area_comp_gt_list.append(area_comp_gt)
    area_comp_sc_list.append(area_comp_sc)
    # num_comp_gt_list.append(num_comp_gt)
    # num_comp_sc_list.append(num_comp_sc)


np.array(area_comp_gt_list).astype(np.float32).tofile('../bins/area_of_components_gt.bin')
np.array(area_comp_sc_list).astype(np.float32).tofile('../bins/area_of_components_sc.bin')
np.array(area_comp_sc_ratio_list).astype(np.float32).tofile('../bins/area_of_components_sc_ratio.bin')
np.array(area_comp_gt_ratio_list).astype(np.float32).tofile('../bins/area_of_components_gt_ratio.bin')
